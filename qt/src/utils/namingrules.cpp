...
bool NamingRules::isValidUsername(const QString &username)
{
    // 检查是否为空
    if (username.isEmpty()) {
        m_message = "Username is empty";
        return false;
    }

    // 检查是否全为空白字符
    if (username.isEmpty()) {
        m_message = "Username is all white characters";
        return false;
    }

    // 检查是否以空格开头
    if (username.startsWith(' ')) {
        m_message = "Username start with white character";
        return false;
    }

    // 检查是否以空格结尾
    if (username.endsWith(' ')) {
        m_message = "Username end with white character";
        return false;
    }
    ...
    if (!regex.match(username).hasMatch()) {
        m_message = "Username contains not allow character";
        return false;
    }
...
bool NamingRules::isValidPassword(const QString &password)
{
    ...
    {
        m_message = "Password must contain letter(s) and digit(s)";
    }
